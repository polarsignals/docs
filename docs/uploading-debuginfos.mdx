...
```
curl -sLO https://github.com/parca-dev/parca-debuginfo/releases/download/v0.5.0/parca-debuginfo_0.5.0_`uname -s`_`uname -m`.tar.gz
```

Verify the checksum:

```
curl -sL https://github.com/parca-dev/parca-debuginfo/releases/download/v0.5.0/checksums.txt | shasum --ignore-missing -a 256 --check
```

And unpack it:

```
tar -xzf parca-debuginfo_0.5.0_`uname -s`_`uname -m`.tar.gz parca-debuginfo
```
