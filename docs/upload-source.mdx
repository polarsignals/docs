...
> Note: For the purpose of demonstration we'll be using [this example repository](https://github.com/polarsignals/sources-demo) with the Go compiler. You can use the pre-built binary checked into the repository, or build it with: `GOOS=linux GOARCH=amd64 go build main.go`.

## Build & Upload Source Archive

First, we need to build the source archive. This `parca-debuginfo` command reads the files referenced in the debuginfo of the given binary and produces an archive with all the files referenced.

```
parca-debuginfo source ./main ./source.tar.zstd
```
