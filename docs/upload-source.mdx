...
* [A service account token and project ID](/docs/generating-tokens.mdx)
...
* `--bearer-token=<your-service-account-token>`: This is your service account authentication token (see [here](generating-tokens) how to create a service account and obtain a token).
* `--grpc-headers=projectID=<your-project-id>`: Specifies which project to upload the source code to.
