...
Locate the input box labeled "Filter by function" located in the visualization bar above the icicle graph. Enter the desired text pattern and hit the "Enter" key. The flame graph will then be updated to display only the samples that include at least one function matching the specified text pattern.
