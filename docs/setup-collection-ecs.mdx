...
1. Create a secret that contains your Polar Signals service account token. In the example below it is injected into the container using [AWS SSM](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-ecs-taskdefinition-secret.html#aws-properties-ecs-taskdefinition-secret-properties), and passed to the agent via the `PARCA_BEARER_TOKEN` environment variable.
2. You'll need to provide your project ID via the command arguments. Replace `<your-project-id>` with your actual project ID.
3. Each of the mount points are important, they cannot be removed, each of them serve an important purpose.
4. It is crucial that the `pidMode` is "host", otherwise the agent cannot perform its functionality.

For instructions on creating a service account token, see the [Generating Tokens](/docs/generating-tokens) documentation.
