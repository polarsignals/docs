...
## Prerequisite
Before you can use Iciclechart with CPU profiling data, you need to be collecting the CPU profiling data with `cpu` or `thread_id` labels. You can do with by adding the following section to the parca agent configuration if you don't have it already:
```yaml
  relabel_configs:
